"""
Tests for scripts/backfill_netsuite_status.py.

Manual-runner pattern, run standalone via
`python3 tests/test_backfill_netsuite_status.py`.

Same "thin one-shot POST, not a list-then-iterate batch runner" shape as
test_backfill_last_video_discovery_at.py -- see that file's own header
comment for the fuller reasoning (this script's real work happens in one
SQL pass server-side, see service.backfill_netsuite_status's docstring,
not per-product from here).
"""
import os
import sys
...
import backfill_netsuite_status as script  # noqa: E402
...
class _FakeSession:
    def __init__(self, payload=None):
        self.payload = payload if payload is not None else {"products_corrected": 0}
        self.post_calls = []
...
def test_backfill_posts_to_the_bulk_endpoint_and_returns_json():
    fake = _FakeSession(payload={"products_corrected": 202})

    result = script.backfill("https://admin.example", "tok", session=fake)

    assert len(fake.post_calls) == 1
    call = fake.post_calls[0]
    assert call["url"] == "https://admin.example/admin/backfill-netsuite-status"
    assert call["headers"]["Authorization"] == "Bearer tok"
    assert result == {"products_corrected": 202}
...
def test_run_calls_backfill_and_returns_its_result(monkeypatch):
    monkeypatch.setattr(
        script, "backfill",
        lambda url, token: {"products_corrected": 202},
    )
...
def test_run_handles_zero_corrections_without_error(monkeypatch):
    """A second/subsequent run after the backfill has already caught
    everything up returns products_corrected=0 -- not an error, just
    nothing left to do (see service.backfill_netsuite_status's
    idempotency docs)."""
    monkeypatch.setattr(
        script, "backfill",
        lambda url, token: {"products_corrected": 0},
    )

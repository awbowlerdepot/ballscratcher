"""
Tests for scripts/rescrape_brunswick_products.py. Exact mirror of
tests/test_rescrape_netsuite_products.py -- see that file's module
docstring for why the injectable-session pattern is used instead of
swapping out sys.modules["requests"] wholesale.

Manual-runner pattern, run standalone via
`python3 tests/test_rescrape_brunswick_products.py`.
"""
import os
import sys
...
import rescrape_brunswick_products as script  # noqa: E402


# --- list_craft_cms_products / rescrape_product: fake injected session ---
...
class _FakeSession:
    ...
    def post(self, url, headers=None, timeout=None):
        ...
        return _FakeResponse({"queued": True, "product_id": url.rsplit("/", 2)[1], "url": "https://brunswickbowling.com/x"})


def test_list_craft_cms_products_paginates():
    fake = _FakeSession(pages=[
        {"items": [{"id": f"prod-{i}", "name": f"Ball {i}"} for i in range(200)]},
        {"items": [{"id": "prod-200", "name": "Ball 200"}]},
    ])

    result = script.list_craft_cms_products("https://admin.example", "tok", page_limit=200, session=fake)

    assert len(result) == 201
    assert len(fake.get_calls) == 2
    assert fake.get_calls[0]["params"] == {"source_platform": "craft_cms", "limit": 200, "offset": 0}
    assert fake.get_calls[1]["params"] == {"source_platform": "craft_cms", "limit": 200, "offset": 200}
    assert fake.get_calls[0]["headers"]["Authorization"] == "Bearer tok"


def test_list_craft_cms_products_single_page_stops_immediately():
    fake = _FakeSession(pages=[{"items": [{"id": "prod-1", "name": "Crown 78U"}]}])
...
def test_rescrape_product_posts_to_rescrape_and_returns_json():
    ...
    assert result == {"queued": True, "product_id": "prod-1", "url": "https://brunswickbowling.com/x"}
...
def test_run_queues_every_listed_product(monkeypatch):
    products = [
        {"id": "prod-1", "name": "Crown 78U"},
        {"id": "prod-2", "name": "Nova"},
    ]
    monkeypatch.setattr(script, "list_craft_cms_products", lambda url, token: products)

    rescraped_calls = []

    def fake_rescrape(url, token, product_id):
        rescraped_calls.append(product_id)
        return {"queued": True, "product_id": product_id, "url": "https://brunswickbowling.com/x"}
...
def test_run_counts_unqueueable_as_skipped_not_errored(monkeypatch):
    """Shouldn't really happen for a craft_cms-sourced product (see run's
    own docstring -- source_platform='craft_cms' always resolves to a real
    ...
    backfill_core_ids.py/rescrape_netsuite_products.py."""
    products = [{"id": "prod-1", "name": "Some Brunswick Ball"}]
    monkeypatch.setattr(script, "list_craft_cms_products", lambda url, token: products)
    monkeypatch.setattr(script, "rescrape_product", lambda url, token, pid:
                         {"queued": False, "reason": "PRODUCT_SCRAPE_QUEUE_URL is not configured on this deployment"})
...
def test_run_tolerates_per_product_rescrape_errors(monkeypatch):
    ...
    this project."""
    products = [{"id": "prod-1", "name": "Crown 78U"}, {"id": "prod-2", "name": "Nova"}]
    monkeypatch.setattr(script, "list_craft_cms_products", lambda url, token: products)
...
def test_run_with_no_craft_cms_products():

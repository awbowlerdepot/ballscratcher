"""
Tests for scripts/rescrape_netsuite_products.py.

Manual-runner pattern, run standalone via
`python3 tests/test_rescrape_netsuite_products.py`.

Same injectable-session pattern as test_backfill_core_ids.py's tests, for
the same reason (get_requests_session touches requests.Session/requests.
adapters.HTTPAdapter/urllib3.util.retry.Retry, so a bare fake module swap
wouldn't cover it) -- this script's list/rescrape/run/retry shape is a
direct copy of backfill_core_ids.py's, just scoped by source_platform
instead of missing_core, so these tests mirror that file's almost
exactly.
"""
import os
import sys
...
import rescrape_netsuite_products as script  # noqa: E402


# --- list_netsuite_products / rescrape_product: fake injected session ---
...
class _FakeSession:
    ...
    def post(self, url, headers=None, timeout=None):
        ...
        return _FakeResponse({"queued": True, "product_id": url.rsplit("/", 2)[1], "url": "https://motivbowling.com/x"})


def test_list_netsuite_products_paginates():
    fake = _FakeSession(pages=[
        {"items": [{"id": f"prod-{i}", "name": f"Ball {i}"} for i in range(200)]},
        {"items": [{"id": "prod-200", "name": "Ball 200"}]},
    ])

    result = script.list_netsuite_products("https://admin.example", "tok", page_limit=200, session=fake)

    assert len(result) == 201
    assert len(fake.get_calls) == 2
    assert fake.get_calls[0]["params"] == {"source_platform": "netsuite", "limit": 200, "offset": 0}
    assert fake.get_calls[1]["params"] == {"source_platform": "netsuite", "limit": 200, "offset": 200}
    assert fake.get_calls[0]["headers"]["Authorization"] == "Bearer tok"


def test_list_netsuite_products_single_page_stops_immediately():
    fake = _FakeSession(pages=[{"items": [{"id": "prod-1", "name": "Sigma Tour Pearl"}]}])
...
def test_rescrape_product_posts_to_rescrape_and_returns_json():
    ...
    assert result == {"queued": True, "product_id": "prod-1", "url": "https://motivbowling.com/x"}
...
def test_run_queues_every_listed_product(monkeypatch):
    products = [
        {"id": "prod-1", "name": "Sigma Tour Pearl"},
        {"id": "prod-2", "name": "Jackal Onyx"},
    ]
    monkeypatch.setattr(script, "list_netsuite_products", lambda url, token: products)

    rescraped_calls = []

    def fake_rescrape(url, token, product_id):
        rescraped_calls.append(product_id)
        return {"queued": True, "product_id": product_id, "url": "https://motivbowling.com/x"}
...
def test_run_counts_unqueueable_as_skipped_not_errored(monkeypatch):
    """Shouldn't really happen for a netsuite-sourced product (see run's
    own docstring -- source_platform='netsuite' always resolves to a real
    queue env var key), but queue_rescrape can still return queued=False
    if that queue's env var isn't actually configured on this deployment
    -- an expected, non-error outcome either way, same convention as
    backfill_core_ids.py."""
    products = [{"id": "prod-1", "name": "Some MOTIV Ball"}]
    monkeypatch.setattr(script, "list_netsuite_products", lambda url, token: products)
    monkeypatch.setattr(script, "rescrape_product", lambda url, token, pid:
                         {"queued": False, "reason": "NETSUITE_PRODUCT_SCRAPE_QUEUE_URL is not configured on this deployment"})
...
def test_run_tolerates_per_product_rescrape_errors(monkeypatch):
    ...
    this project."""
    products = [{"id": "prod-1", "name": "Sigma Tour Pearl"}, {"id": "prod-2", "name": "Jackal Onyx"}]
    monkeypatch.setattr(script, "list_netsuite_products", lambda url, token: products)
...
def test_run_with_no_netsuite_products():

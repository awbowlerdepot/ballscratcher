...
# public_api's copy. See public_api/service.py's module-level comment
# above its own estimate_oil_motion for the full reasoning behind every
# constant here, and migration 012's header comment for why this hook
# exists at all -- Al's direct ask: "estimate on scrape if not set".
# --------------------------------------------------------------------
...
def _reference_differential(skus: list):
    ...
    product overall, straight off the just-parsed skus list. Same
    15lb-preferred convention as public_api._reference_sku."""
...
def upsert_product(conn, brand_id: str, status: str, parsed: dict) -> dict:
    ...
    with conn.cursor() as cur:
        ...
        # Estimate-on-scrape plotter position (migrations 011/012) --
        # only ever touches a product with NO plotter position yet
        # ("where oil_rating is null" guards against clobbering a chart
        # match or an admin's manual correction on a rescrape). core_type
        # is re-read from the cores row rather than trusting parsed's own
        # value directly, since the cores row is the coalesced,
        # authoritative one (may have been filled in by a different
        # platform's scrape of the same shared core, or an admin).
        core_type = None

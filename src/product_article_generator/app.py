...
INVOCATION CAP (2026-09-06): the scheduled DailySchedule sweep (see
handler, list_products_needing_article) used to fetch and process EVERY
eligible product in one invocation with no limit. Al asked "how many will
get generated by the daily sweep" and a direct SQL count came back 53 --
each of which costs one Bedrock text call plus up to NUM_GEMINI_
CANDIDATES_PER_VARIANT * 2 (6) Gemini image calls, several of which retry
with backoff on a 429 (see v6 above). At 600s (this function's Timeout),
53 products was a real risk of the invocation timing out mid-run and
losing whatever in-flight product's work hadn't been committed yet --
this was flagged as a known risk before ever being fixed, and this same
session separately confirmed the account's Lambda concurrency ceiling
sits at only 10 (see template.yaml's AdminApiFunction comment), making it
more important than before that any one invocation actually finish
within its window rather than run long and hold a scarce execution slot.
...
applies it as a plain SQL LIMIT (already ordered by p.id, so this is
deterministic -- the same lowest-id products lead every run rather than a
random subset). handler() reads MAX_PRODUCTS_PER_INVOCATION (default
DEFAULT_MAX_PRODUCTS_PER_INVOCATION below) for the scheduled {}/
{"batch": true} shape, but a manual invoke can override it with an
explicit {"batch": true, "limit": N} -- useful for deliberately powering
through a backlog (like the confirmed 53) in one manual run without
permanently raising the daily default. Products left over after a capped
run are untouched (list_products_needing_article's "no article row yet"
gate still applies), so they simply roll into the next scheduled run --
same non-destructive partial-completion behavior this was already
relying on before the cap existed, just now bounded on purpose instead of
by accident.

DEFAULT_MAX_PRODUCTS_PER_INVOCATION=10 is a starting estimate, not a
measured one -- there's no real per-product timing data for this
function yet (unlike video_discovery's YouTube-quota-driven number).
10 products x a generous worst-case ~45s each (text + 6 image calls with
retries) is ~450s, leaving real margin under the 600s timeout; tune this
via this function's own CloudWatch Duration metric once a few real
invocations have run under the cap.
"""
import json
import logging
import os
...
# See "INVOCATION CAP" in this module's own docstring -- bounds how many
# eligible products the scheduled DailySchedule sweep processes in one
# invocation, so a large backlog can't run this function's 600s Timeout
# out mid-loop. Not a measured number yet; revisit via CloudWatch
# Duration once real capped invocations have run. A manual {"batch":
# true, "limit": N} invoke overrides this for deliberate backlog
# catch-up (see handler).
DEFAULT_MAX_PRODUCTS_PER_INVOCATION = 10
...
def list_products_needing_article(conn, max_products: int = None) -> list:
    ...
    max_products (see "INVOCATION CAP" in this module's own docstring):
    when given, applied as a plain SQL LIMIT on top of the existing
    `order by p.id` -- deterministic (the same lowest-id eligible
    products lead every capped run) rather than an arbitrary subset.
    Left as None for the on-demand/uncapped case (there isn't one right
    now, but this keeps the function usable that way without a sentinel
    like 0 or -1)."""
    with conn.cursor() as cur:
        query = """
...
def build_article_prompt(product: dict, siblings: list) -> str:
    ...
    return (
        ...
        "This article is about the EXACT ball named in \"Name\" below -- refer "
        "to it only by that exact name and the exact coverstock/finish given in "
        "\"Coverstock\"/\"Factory finish\" below. Bowling balls are frequently "
        "sold in multiple separate editions of the same base name (Solid, "
        "Pearl, Hybrid, Particle, etc.) with materially different performance. "
        "If the video content below turns out to actually be about a "
        "DIFFERENT edition of this ball than the one specified (a different "
        "coverstock finish/color than \"Coverstock\"/\"Factory finish\" give), "
        "do not borrow that edition's name or finish-specific performance "
        "claims for this article -- write only what's genuinely applicable to "
        "THIS edition, and lean more on the specs above than the video if they "
        "conflict. Never introduce an edition/finish word (\"Pearl\", "
        "\"Solid\", \"Hybrid\", \"Particle\") into title/hook/performance_"
        "summary/verdict/faq that does not already appear in this product's "
        "own Name/Coverstock/Factory finish above, even if that word appears "
        "in a video title or transcript below.\n\n"
        ...
        "\"Origin\" might evoke something cosmic or primordial. If the name "
        "evokes a concrete creature, object, or symbol -- an animal "
        "(\"Black Widow\" -> a spider, its web), a substance (\"Venom\" -> a "
        "snake, fangs, dripping venom), a place or artifact -- naming that "
        "literal thing and describing it as a visible element in the scene "
        "(a spiderweb draped across the backdrop, a coiled snake worked "
        "into the surface the ball rests on) is a strong, encouraged "
        "choice, not just a color palette or mood inspired by it. That "
        "said, a mood/color-only treatment is a perfectly fine choice too "
        "when it genuinely serves the scene better -- this isn't a rule to "
        "force a literal creature into every image, just a nudge not to "
...
def build_gemini_scene_prompt(product: dict, article: dict, variant: str,
    ...
    came out "plain jane." Al's own framing, on seeing the first draft
    of this fix: "i don't know that we want that to be literalism just
    was suprised to not see spider webs in the black widow one for
    sure" -- a nudge that an obvious literal element shouldn't be
    skipped, NOT a hard rule that every theme must render literally
    (some names have no clean literal referent, and forcing one would
    produce worse, more literal-minded images). Addressed at both
    points a theme could get lost: the visual_theme field itself
    (generated by build_article_prompt, Bedrock/Claude) now calls out
    naming a concrete creature/object/symbol as a "strong, encouraged
    choice... not just a color palette or mood inspired by it" when the
    name evokes one, while explicitly saying a mood/color-only
    treatment is "a perfectly fine choice too... this isn't a rule to
    force a literal creature into every image, just a nudge not to
...
def handler(event, context):
    ...
    v8 (2026-09-06), see "INVOCATION CAP" in this module's own docstring:
    the batch sweep path ({}/{"batch": true}) now caps how many eligible
    products it fetches via list_products_needing_article's max_products,
    read from MAX_PRODUCTS_PER_INVOCATION (falls back to DEFAULT_MAX_
    PRODUCTS_PER_INVOCATION when unset) -- or an explicit event["limit"]
    when one is passed, letting a manual invoke override the scheduled
    default for deliberate backlog catch-up. The on-demand {"product_id":
    ...
    try:
        ...
        max_products = int(
            event.get("limit")
            or os.environ.get("MAX_PRODUCTS_PER_INVOCATION", DEFAULT_MAX_PRODUCTS_PER_INVOCATION)
        )
        product_ids = list_products_needing_article(conn, max_products=max_products)
        logger.info("Generating articles for %d product(s) (cap=%d)", len(product_ids), max_products)
        results = []
